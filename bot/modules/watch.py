    ...
    if "entries" in result:
        ...
        for frmt in formats:
            ...
            if (
                ...
                or quality in formats_dict
                and formats_dict[quality][1] < frmt["tbr"]
            ):
        ...
        for forDict in formats_dict:
            ...
            if qual_fps_ext[1] != "":
                ...
                video_format = (
                    f"bv*[height={qual_fps_ext[0]}][ext={qual_fps_ext[2]}]+ba/b"
                )
            buttonName = (
                f"{forDict} ({get_readable_file_size(formats_dict[forDict][0])})"
            )
...
def select_format(update, context):
    ...
    if user_id != uid:
        ...
        ydl = YoutubeDLHelper(listener)
        threading.Thread(
            target=ydl.add_download, args=(link, f"{DOWNLOAD_DIR}{task_id}", name, qual)
        ).start()
