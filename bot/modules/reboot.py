...
from bot import OWNER_ID, app, bot
from bot.helper import check_heroku


@app.on_message(
    filters.command(["reboot", f"reboot@{bot.username}"]) & filters.user(OWNER_ID)
)
@check_heroku
...
@app.on_message(
    filters.command(["shutdown", f"shutdown@{bot.username}"]) & filters.user(OWNER_ID)
)
@check_heroku
