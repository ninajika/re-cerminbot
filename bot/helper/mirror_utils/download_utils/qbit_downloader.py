    ...
    def add_torrent(self, link, dire, listener, qbitsel):
        ...
        try:
            ...
            if BASE_URL is not None and qbitsel:
                if not self.is_file:
                    meta = sendMessage(
                        "Downloading Metadata, wait then you can select files or mirror Torrent file if it have low seeders",
                        listener.bot,
                        listener.update,
                    )
                    while True:
    ...
    def update(self):
        ...
        try:
            ...
            if tor_info.state == "metaDL":
                ...
                self.client.auth_log_out()
                self.updater.cancel()
            elif tor_info.state != "checkingUP" and (
                tor_info.state == "uploading" or tor_info.state.lower().endswith("up")
            ):
                self.client.torrents_pause(torrent_hashes=self.ext_hash)
                if self.qbitsel:
